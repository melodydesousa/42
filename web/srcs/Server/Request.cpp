...
Request::Request() : b_method(false), bad_request(false), header_recv(false), Content_length(false), All_recv(false) , loc(NULL)
{
...
void Request::request_to_cgi( void )
{
	...
	{	
		...
		if (this->GetContentLength()) // a recup de Clement
		{
		...
		else if (this->cgi.getMethod() == "POST")
		{
